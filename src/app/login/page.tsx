...
export default function LoginPage() {
  ...
  const google = async () => {
    setError(null);
    setBusy(true);
    // Supabase redirects to Google, then back to the app; AuthProvider picks up
    // the session and the effect above sends the user to "/".
    ...
    <div className="flex min-h-dvh items-center justify-center p-6">
      <div className="grid w-full max-w-4xl items-center gap-12 lg:grid-cols-[1.1fr_1fr]">
        ...
        <button
          ...
          onClick={google}
          disabled={busy}
          className="mb-4 flex h-[52px] w-full items-center justify-center gap-3 rounded-2xl border border-line bg-white/[0.04] text-sm font-semibold text-fg transition hover:bg-white/[0.08] disabled:opacity-50"
        >
