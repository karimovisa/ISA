...
// ISA — the 30-second evening reflection (§7). Two taps, not a questionnaire. It
// only asks once a day, only in the evening, and only follows up when the day was
// actually off — because "why?" is the one answer the engine can't derive. Sleep
// is NOT asked here anymore: it's logged in the morning-first Sleep card.
...
export function DailyCheckin() {
  ...
  const title = verdict === "off" ? t("What got in the way?") : t("How did today go?");
  const sub = verdict === "off"
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mb-4">
        <GlassCard className="p-5">
          <div className="mb-3 flex items-start justify-between gap-3">
            ...
            <button onClick={dismiss} aria-label={t("Cancel")} className="shrink-0 rounded-lg p-1 text-muted transition hover:text-fg">
              <X size={15} />
            </button>
          </div>
