...
```mermaid
flowchart TD
    A[Proposed feature / insight / change] --> C1{1. Does it CONNECT<br/>to the rest of the life?}
    C1 -- No --> R[Reject or redesign]
    C1 -- Yes --> C2{2. Does it create UNDERSTANDING,<br/>not just more data?}
    C2 -- No --> R
    C2 -- Yes --> C3{3. Does it REDUCE the user's<br/>cognitive load & feel CALM?}
    C3 -- No --> R
    C3 -- Yes --> C4{4. Is it HONEST —<br/>no faked intelligence/certainty?}
    C4 -- No --> R
    C4 -- Yes --> C5{5. Does it TEACH /<br/>make the user more capable?}
    C5 -- No --> R
    C5 -- Yes --> C6{6. Does it respect the<br/>WHOLE PERSON & their values?}
    C6 -- No --> R
    C6 -- Yes --> C7{7. Does it protect PRIVACY<br/>& user ownership?}
    C7 -- No --> R
    C7 -- Yes --> C8{8. Will it AGE WELL —<br/>right for a 10-year user?}
    C8 -- No --> R
    C8 -- Yes --> C9{9. Does it avoid every<br/>WHAT-ISA-IS-NOT trap?}
    C9 -- No --> R
    C9 -- Yes --> P[Passes — build it the ISA way]
```
